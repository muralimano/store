...
public class User_Cart_DB {
    ...
    // Table Name
    public static final String TABLE_CART = "User_Cart1";
    // Table Columns
    ...
    public static String createTableCart() {
        ...
        return "CREATE TABLE "+ TABLE_CART +
                "(" +
                    ...
                    CART_PRODUCT_NAME               + " TEXT PRIMARY KEY," +
    ...
    public String getUserData(String productname){
        // get and open SQLiteDatabase Instance from static method of DB_Manager class
        db = DB_Manager.getInstance().openDatabase();

        String proname="";

        Cursor cursor =  db.rawQuery( "SELECT products_name FROM "+ TABLE_CART +" WHERE "+ CART_PRODUCT_NAME +" =?", new String[] {productname});
    ...
    public void updateCartQty(String productname,String Qty,String price) {
        // get and open SQLiteDatabase Instance from static method of DB_Manager class
        db = DB_Manager.getInstance().openDatabase();

        ContentValues values = new ContentValues();

        values.put(CART_PRODUCT_QUANTITY,Qty);
        values.put(CART_PRODUCT_TOTAL_PRICE,price);

        Integer result=db.update(TABLE_CART, values, CART_PRODUCT_NAME +" = ?", new String[]{productname});
        if(result==0){
    ...
    public void deleteCartItem(String cart_productname) {
        // get and open SQLiteDatabase Instance from static method of DB_Manager class
        db = DB_Manager.getInstance().openDatabase();

        db.delete(TABLE_CART, CART_PRODUCT_NAME +" = ?", new String[]{String.valueOf(cart_productname)});
        db.delete(TABLE_CART_ATTRIBUTES, CART_TABLE_ID +" = ?", new String[]{String.valueOf(cart_productname)});
