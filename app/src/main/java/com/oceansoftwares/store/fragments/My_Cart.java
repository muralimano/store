...
import com.oceansoftwares.store.activities.adapters.CartItemsAdapter;
...
//                                .addToBackStack(getString(R.string.actionAddresses)).commit();
    ...
    public static void UpdateQTY(String productname,String Qty,String price) {
        User_Cart_DB user_cart_db = new User_Cart_DB();
        user_cart_db.updateCartQty
                (
    ...
    public static int getCartSize() {
        ...
        for (int i=0;  i<cartItems.size();  i++) {
            productname=cartItems.get(i).getCustomersBasketProduct().getProductsName();
            mProductname.add(productname);
            //cartSize += cartItems.get(i).getCustomersBasketProduct().getCustomersBasketQuantity();
        }

        HashSet<String> hashSet = new HashSet<String>();
        hashSet.addAll(mProductname);
        mProductname.clear();
        mProductname.addAll(hashSet);

        cartSize=mProductname.size();
